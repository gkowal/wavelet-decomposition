...
!
  subroutine hdf5_init
    ...
    use params, only : idir, ftype, fnumber, ffmt, csnd
...
! generate filename
!
    write(frmt, "(a,a,a)") '(a,', trim(ffmt), ')'
    write(fl,trim(frmt)) trim(idir), ftype, fnumber, 0
...
!
      do i = 0, nattr - 1
        ...
        case('csnd')
          call h5aread_f(aid, H5T_NATIVE_REAL, csnd, am, err)
        end select
...
! generate filename
!
    write(fl,trim(frmt)) trim(idir), ftype, fnumber, n
